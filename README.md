...
Input file h1b_input.csv is raw data converted into semicolon separated (";") format.
...
Simply execute ./run.sh in the main repo then the output files will be generated in the output folder. The python script only recognize input file name h1b_input.csv in the input folder and the output file names are fixed. Need to make adjustment to h1b_counting.py in src in order to support files with different names or different desired output. The script only support files with semicolon separated (";") format. The first row of the file needs to be the column names seperated by semicolon.
